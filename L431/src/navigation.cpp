...
BearingEstimate estimate_bearing(const InfraredSample& sample) noexcept {
    ...
    float x = 0.0F;
    float y = 0.0F;
    for (std::size_t index = 0U; index < sample.strength.size(); ++index) {
        const float weight = static_cast<float>(sample.strength[index]);
        const float angle = static_cast<float>(index) * kSensorStepDeg * kPi / 180.0F;
